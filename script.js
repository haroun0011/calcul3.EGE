...
// ⬇ Paste your Google Client ID here (see GOOGLE_SETUP.html)
const GOOGLE_CLIENT_ID = '924883918269-fits7feuqg0odpodji48n2tr46hk5un8.apps.googleusercontent.com';
...
function handleCredentialResponse(response) {
    // Decode the JWT token Google returns — no server needed
    const payload  = JSON.parse(atob(response.credential.split('.')[1]));
    const name     = payload.name  || 'Unknown';
    const email    = payload.email || '';
    const visitorId = name + ' (' + email + ')';

    sessionStorage.setItem(SESSION_KEY, visitorId);
    saveVisitor(visitorId);
    showApp();
}

/* ---------- Render the Google button once the GSI script loads ---------- */
function initGoogleSignIn() {
    if (typeof google === 'undefined') {
        // Script not loaded yet — retry in 300ms
...
/* ---------- Show the main app, hide the gate ---------- */
function showApp() {
    document.getElementById('nameGate').style.display = 'none';
    document.getElementById('mainApp').style.display  = 'block';
}

/* ---------- Save / update visitor with latest grades ---------- */
function saveVisitor(visitorId) {
...
document.addEventListener('DOMContentLoaded', function() {
    // If already signed in this session, skip the gate
